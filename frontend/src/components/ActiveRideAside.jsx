...
/**
 * Home aside for someone who already has a ride in progress: live status and a
 * one-tap return to the live map, instead of the idle hero.
 */
export default function ActiveRideAside({ ride, role }) {
  ...
  return (
    <aside
      ...
      style={{
        background: "radial-gradient(120% 120% at 70% 10%, #0f7a63 0%, #0b5c4b 45%, #083b31 100%)",
      }}
    >
      <div className="bg-grid absolute inset-0 opacity-[0.06]" aria-hidden="true" />
      <div
        className="absolute -left-24 top-1/3 h-72 w-72 animate-float rounded-full bg-brand-500/25 blur-3xl"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 flex h-full flex-col justify-center p-10 xl:p-14">
        <div className="animate-fade-up w-full max-w-md">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-medium text-white/85">
            ...
            Live · {statusLabel(ride.status)}
          </span>

          <h2 className="mt-5 text-3xl font-semibold leading-tight tracking-tight text-white">
            {copy.title}
          </h2>
          <p className="mt-2 text-sm text-white/70">{copy.detail}</p>
          ...
          <div className="glass mt-6 rounded-3xl border border-white/15 p-5 shadow-floating">
              ...
              <span className="text-right">
                ...
                {ride.eta_minutes != null && (
                  <span className="block text-xs text-white/60">
                    {formatDuration(ride.eta_minutes)} away
                  </span>
                )}
              </span>
            </div>
            ...
            <div className="relative mt-4 pl-6">
              <span
                aria-hidden
                className="absolute left-[5px] top-2 h-[calc(100%-1rem)] w-px border-l border-dashed border-white/30"
              />
