...
/**
 * A second booking offered to a driver already on a trip.
 *
 * The card leads with the *marginal* arithmetic — what this adds in money, in
 * minutes, and the rate between them — rather than the new fare on its own. A
 * driver who cannot see that pooling raises their earnings per hour declines
 * every offer, and the feature dies on the supply side rather than the demand
 * side.
 */
export default function PoolOfferCard({ offer, onAccept, onDismiss, busy, secondsLeft }) {
    ...
    <Card className="border-primary/40 bg-primary/5 p-4 shadow-floating">
      <div className="flex items-start justify-between gap-2">
        <span className="flex items-center gap-2">
          <Badge tone="success">On your way</Badge>
      ...
      {/* The marginal case, which is the only case that matters to a driver. */}
      <dl className="mt-3 grid grid-cols-3 gap-2 rounded-xl bg-surface px-3 py-2.5 text-center">
        ...
        <div className="border-x border-border">
          <dt className="text-[0.65rem] uppercase tracking-wide text-subtle">Extra time</dt>
          <dd className="mt-0.5 text-sm font-semibold text-foreground">
            +{formatDuration(offer.added_minutes)}
          </dd>
        </div>
        <div>
          <dt className="text-[0.65rem] uppercase tracking-wide text-subtle">Per minute</dt>
